...
object Assignment1Util {
  ...
  def loadTokenizationQuestionDocs(dataDir: File) = {
    ...
    val jlive = OHHLA.allAlbumDirs(new File(artistDir, OHHLA.JLive.id)) flatMap OHHLA.loadDirRaw
    val roots = OHHLA.allAlbumDirs(new File(artistDir, OHHLA.Roots.id)) flatMap OHHLA.loadDirRaw
    jlive ++ roots
  }
